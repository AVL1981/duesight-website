...
def test_public_surface_gate_blocks_links_to_filtered_local_artifacts(tmp_path):
    sample = tmp_path / "sample-report-nlist"
    sample.mkdir()
    (sample / "hub.html").write_text(
        '<a href="index.html">Kort</a><a href="pipeline-manifest.json">Manifest</a>',
        encoding="utf-8",
    )
    (sample / "index.html").write_text("<h1>Sample</h1>", encoding="utf-8")

    findings = public_surface_gate.scan_public_surface(tmp_path, strict=True)

    assert any("missing local link target: pipeline-manifest.json" in finding for finding in findings)
    assert not any("missing local link target: index.html" in finding for finding in findings)
